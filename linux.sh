...
echo "4) 安装宝塔面板11版本&LNMP环境（PHP5.6,Mysql5.6,Nginx1.18）"
echo "5) 安装宝塔面板（11版本）"
echo "---------------------------"
echo "6) 安装魔方云"
echo "7) 安装魔方云（备用）"
...
case $choice in
   ...
   echo "nameserver 223.6.6.6" >> /etc/resolv.conf
    ...
    11)
        echo "正在换源"
        curl -sSL https://linuxmirrors.cn/main.sh -o main.sh && chmod +x main.sh && ./main.sh
        ;;
    12)
        echo "正在换源"
    ...
    14)
        echo "正在安装Docker"
        sudo curl -O https://raw.githubusercontent.com/Utuobw/Linux-toolbox/main/docker.sh && sudo bash docker.sh
        ;;
    15)
        echo "正在安装宝塔面板破解版"
